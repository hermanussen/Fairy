...
namespace Fairy.Tests
{
    ...
    public abstract class TestsBase
    {
        ...
        protected Compilation GetGeneratedOutput(string source, IDictionary<string, string> additionalFilesAndContents, List<Diagnostic>? diagnostics = null)
        {
            ...
            var additionalTexts = additionalFilesAndContents.Select(i => new AdditionalTextYml(i.Key, i.Value)).Cast<AdditionalText>();
            ...
            ISourceGenerator generator = new FairyGenerator();
            ...
            var output = outputCompilation
                .SyntaxTrees
                .Skip(outputCompilation.SyntaxTrees.Count() - additionalFilesAndContents.Where(a => a.Key.EndsWith(".yml", StringComparison.InvariantCultureIgnoreCase)).Count());
